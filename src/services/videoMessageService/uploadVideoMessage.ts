...
export const uploadVideoMessage: VideoMessageServiceHandlers["UploadVideoMessage"] =
  async (call: ServerDuplexStream<VideoMessageChunk, VideoMessageMetadata>) => {
    ...
    try {
      ...
      call.on("end", async () => {
        ...
        const videoMessage: VideoMessage =
          await prismaClient.videoMessage.upsert({
            where: { id: messageId },
